...
func main() {
	...
	var ldFlags = fmt.Sprintf("-X 'github.com/liserjrqlxue/version.gitDescribe=%s' -X 'github.com/liserjrqlxue/version.buildStamp=%s' -X 'github.com/liserjrqlxue/version.golangVersion=%s' %s", gitDescribe, date, goVersion, *ldflags)

	var args = []string{"build", "-ldflags", ldFlags}

	if *x {
		args = append(args, "-x")
	}

	var build = exec.Command("go", args...)
